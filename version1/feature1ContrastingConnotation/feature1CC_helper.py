...
def get_trigrams(inputFile, neg, pos):
	...
	for row in df:
		...
		if(row[0] == '0'):
			...
			# positive
			pos_triwords = trigrams(row[1].split(" "))
			print pos_triwords
...
get_trigrams(input_file, neg_trigram_data, pos_trigramdata)



generate_scores(output_bigram_file, "positive_bigrams.csv", "negative_bigrams.csv", bigram_list)
generate_scores(output_trigram_file, "positive_trigrams.csv", "negative_trigrams.csv", trigram_list)
