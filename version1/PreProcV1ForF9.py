...
def main():
    ...
    pwd = os.getcwd();
    sys.path.append(pwd)
    norm_in = pwd + "/newdatasetForF9/normal_with_past"
    norm_out = pwd + "/newdatasetForF9/normal_with_past_PP"
    sarc_in = pwd + "/newdatasetForF9/sarcastic_with_past"
    sarc_out = pwd + "/newdatasetForF9/sarcastic_with_past_PP"
    
    # preprocessing normal tweets
    for f in os.listdir(norm_in):
        inputFile = io.open(os.path.join(norm_in,f),"r")
        outputFile = io.open(os.path.join(norm_out,f),"w")
